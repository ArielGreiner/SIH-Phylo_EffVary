...
#^gives weird warning messages...
  ...
  #SES measures
  Data_storage$sesMPD_abund[Data_storage$Dispersal==DispV[i] & Data_storage$ReplicateNum==j & Data_storage$Scale == "Regional"]<-mean(ses.mpd(samp = com_data,dis = cophenetic(SIH_data[["phylo",i]]),null.model = "sample.pool",abundance.weighted = T, runs = 999)$mpd.obs.z)

Data_storage$sesMNTD_abund[Data_storage$Dispersal==DispV[i] & Data_storage$ReplicateNum==j & Data_storage$Scale == "Regional"]<-mean(ses.mntd(samp = com_data,dis = cophenetic(SIH_data[["phylo",i]]),null.model = "sample.pool",abundance.weighted = T, runs = 999)$mntd.obs.z)
  }
  }
require(dplyr) 
...
Mean_beta_MNTDabund=mean(beta_MNTDabund,na.rm=T), SD_beta_MNTDabund=sd(beta_MNTDabund,na.rm=T), Mean_sesMPD_abund = mean(sesMPD_abund,na.rm=T), SD_sesMPD_abund = sd(sesMPD_abund,na.rm=T), Mean_sesMNTD_abund = mean(sesMNTD_abund,na.rm=T), SD_sesMNTD_abund = sd(sesMNTD_abund,na.rm=T))
