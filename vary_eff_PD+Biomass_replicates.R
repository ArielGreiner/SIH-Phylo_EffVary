...
for(i in 1:length(DispV)){
 #BIOMASS
  LFunc_rate1<-apply(SIH_data[["Abund",i]]) #calculates the amount of each function in each patch based on the abundances from the SIH model
  LFunc_rate<-array(t(LFunc_rate1),dim=c(nrow(SIH_data[["Abund",1]]),1,npatches)) #arranges the functions into an array that is easier to use
...
#Plot Biomass
ggplot(Data_storage_avg,aes(x=Dispersal,y=Biomass,color=Function,group=Function,linetype=Function))+ #defines the variables that you are plotting
  geom_line(size=2)+ #plots data as lines
